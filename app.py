...
st.sidebar.markdown("This panel lets you control how the analysis runs.")

uploaded_file = st.sidebar.file_uploader("📁 Upload Dataset", type=["csv"], help="Upload your own dataset with 'target' column.")
feature_method = st.sidebar.selectbox("🧠 Feature Selection", ["Both", "BAT", "CFS"], help="Choose how features are selected.")
k_value = st.sidebar.slider("🔢 K Value for KNN", min_value=1, max_value=15, value=7, help="Number of neighbors in KNN.")
test_size = st.sidebar.slider("📊 Test Size (%)", min_value=10, max_value=50, value=20, step=5, help="Test data percentage.")
...
if run_analysis:
    ...
    if feature_method in ["BAT", "Both"]:
        ...
        bat_acc, bat_prec, bat_rec, bat_f1, bat_cm, model_bat = train_and_evaluate(X_train_bat, X_test_bat, y_train, y_test, k_value)
        results["BAT"] = [bat_acc, bat_prec, bat_rec, bat_f1, bat_cm, bat_idx]
        trained_model = model_bat
    ...
    if feature_method in ["CFS", "Both"]:
        ...
        cfs_acc, cfs_prec, cfs_rec, cfs_f1, cfs_cm, model_cfs = train_and_evaluate(X_train_cfs, X_test_cfs, y_train, y_test, k_value)
        results["CFS"] = [cfs_acc, cfs_prec, cfs_rec, cfs_f1, cfs_cm, cfs_idx]
        if trained_model is None:
            trained_model = model_cfs
    ...
    if show_accuracy_chart:
        ...
        st.plotly_chart(fig)
        st.markdown("**Interpretation:** This chart shows which feature selection method yields higher accuracy.")
    ...
    if show_confusion:
        for method in results:
            ...
            st.pyplot(plt.gcf())
            st.markdown("**Interpretation:** Diagonal values are correct predictions; off-diagonal values are misclassifications.")
